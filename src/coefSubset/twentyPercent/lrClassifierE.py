...
with open(pathToOutput + 'modelE.pkl', 'wb') as h:
    pickle.dump(clf, h)

with open(pathToOutput + 'trainingSetE.txt', 'w') as i:
    i.write('Training set:\n')
    for pdbID in trainFiles:
        i.write(pdbID + '\n')
    i.write('\nJob time: ' + str((tic - toc) / 60 / 60) + ' hours')

with open(pathToOutput + 'standardScalerE.pkl', 'wb') as j:
    pickle.dump(scaler, j)

bestCoefficient = clf.coef_
coefDf = pd.DataFrame(bestCoefficient, columns = feature_names)

with open(pathToOutput + 'coefficientsE.csv', 'w') as f:
    coefDf.to_csv(f)
