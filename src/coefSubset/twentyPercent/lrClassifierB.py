...
with open(pathToOutput + 'modelB.pkl', 'wb') as h:
    pickle.dump(clf, h)

with open(pathToOutput + 'trainingSetB.txt', 'w') as i:
    i.write('Training set:\n')
    for pdbID in trainFiles:
        i.write(pdbID + '\n')
    i.write('\nJob time: ' + str((tic - toc) / 60 / 60) + ' hours')

with open(pathToOutput + 'standardScalerB.pkl', 'wb') as j:
    pickle.dump(scaler, j)

bestCoefficient = clf.coef_
coefDf = pd.DataFrame(bestCoefficient, columns = feature_names)

with open(pathToOutput + 'coefficientsB.csv', 'w') as f:
    coefDf.to_csv(f)
