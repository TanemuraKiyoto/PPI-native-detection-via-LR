...
with open(pathToOutput + 'modelJ.pkl', 'wb') as h:
    pickle.dump(clf, h)

with open(pathToOutput + 'trainingSetJ.txt', 'w') as i:
    i.write('Training set:\n')
    for pdbID in trainFiles:
        i.write(pdbID + '\n')
    i.write('\nJob time: ' + str((tic - toc) / 60 / 60) + ' hours')

with open(pathToOutput + 'standardScalerJ.pkl', 'wb') as j:
    pickle.dump(scaler, j)

bestCoefficient = clf.coef_
coefDf = pd.DataFrame(bestCoefficient, columns = feature_names)

with open(pathToOutput + 'coefficientsJ.csv', 'w') as f:
    coefDf.to_csv(f)
