...
with open(pathToOutput + 'bestParamIDENTIFIER.json', 'w') as g:
    json.dump(randomized_search.best_estimator_.get_params(), g)

with open(pathToOutput + 'modelIDENTIFIER.pkl', 'wb') as h:
    pickle.dump(randomized_search, h)

with open(pathToOutput + 'trainingSetIDENTIFIER.txt', 'w') as i:
    i.write('Training set:\n')
    for pdbID in fileNames[:train_file_number]:
        i.write(pdbID + '\n')
    i.write('\nJob time: ' + str((tic - toc) / 60 / 60) + ' hours')

with open(pathToOutput + 'standardScalerIDENTIFIER.pkl', 'wb') as j:
...
with open(pathToOutput + 'coefficientsIDENTIFIER.csv', 'w') as f:
    coefDf.to_csv(f)
